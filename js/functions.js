...
function TTS() {
    if (eventsToTrigger.length != 0) {
        var first = eventsToTrigger.shift()
        console.log("PLAYING: " + first.text);

        TextToSpeech(first.text, first.name);
    }
...
function FetchEvent() {
    ...
    channel.bind('onChat', async function (data) {
        ...
        if (document.getElementById("ip7").checked == true) {
            var cleanCom = findEmoji(input);

            if (cleanCom.localeCompare("") == 0) {

            } else {
                if (foundname.includes("adisaaaa")) {


                    var newEvent = new Event("adi", findEmoji(input));
                    eventsToTrigger.push(newEvent);
                } else {
                    console.log("ADDED EVENT:" + input);
                    var newEvent = new Event(foundname.substring(0, 4), findEmoji(input));
                    eventsToTrigger.push(newEvent);
                }
            }
    ...
    channel.bind('onGift', function (data) {
        ...
        if (data.message != "undefined") {
            ...
            if (test == 0) {
                console.log('PLAYING THIS ANIM1');
                Animation("New Gift from " + foundname.substring(0, 19) + ":" + "<br>" + "<b>" + "Tip Jar: Luca couldn't find the value" + "</b>");
            } else if (foundValue > document.getElementById("ipMinimumBarrs").value) {
                console.log('PLAYING THIS ANIM2');
...
function findEmoji(input) {
    ...
    var emojiRegexp = /([\uE000-\uF8FF]|\uD83C[\uDC00-\uDFFF]|\uD83D[\uDC00-\uDFFF]|[\u2694-\u2697]|\uD83E[\uDD10-\uDD5D])/g;
    input = input.replace(emojiRegexp, '');
    return input;
}